...
def determine_type(amount, category):
    if category == "skip" or category is None:
        return None
    if amount > 0:
        return "income"
    elif amount < 0:
...
def load_and_preprocess_csv(file_path):
    ...
    df["amount"] = df["Amount DR/Amount CR"].replace(",", ".", regex=True).astype(float)
    ...
    df["type"] = df.apply(
        lambda row: determine_type(row["amount"], row["category"]), axis=1
    )
...
def ask_user_for_category(row):
    ...
    user_input = (
        input(f"  Enter category {valid_categories} or 'skip': ").strip().lower()
    )
    log(f"User input category: {user_input}", "DEBUG")

    return user_input if user_input in valid_categories else "skip"
...
def main():
    ...
    df_sqlite = pd.read_sql_query(
        "SELECT type, category, SUM(amount) as total_amount FROM transactions GROUP BY type, category",
        conn,
    )
